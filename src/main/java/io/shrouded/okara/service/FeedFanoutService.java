...
public class FeedFanoutService {
    ...
    public Mono<Void> processFeedEvent(String eventJson) {
        try {
            FeedEventPublisher.FeedEvent event = objectMapper.readValue(eventJson, FeedEventPublisher.FeedEvent.class);
    ...
    private Mono<Void> handlePostCreated(FeedEventPublisher.FeedEvent event) {
    ...
    private Mono<Void> handlePostUpdated(FeedEventPublisher.FeedEvent event) {
    ...
    private Mono<Void> handlePostDeleted(FeedEventPublisher.FeedEvent event) {
    ...
    private Mono<Void> handleUserFollowed(FeedEventPublisher.FeedEvent event) {
        String followerId = (String) event.getMetadata().get("followerId");
        String followedId = (String) event.getMetadata().get("followedId");
    ...
    private Mono<Void> handleUserUnfollowed(FeedEventPublisher.FeedEvent event) {
        String followerId = (String) event.getMetadata().get("followerId");
        String unfollowedId = (String) event.getMetadata().get("unfollowedId");
    ...
    private Mono<Void> processFanoutDiscoveryBatch(Feed post, List<User> users) {
        ...
        for (User user : users) {
            feedOperations.add(
                getUserFeed(user.getId(), UserFeed.FeedType.PERSONAL)
                    .map(userFeed -> {
                        userFeed.addItem(new UserFeed.FeedItem(post));
                        return userFeed;
                    })
                    .onErrorResume(e -> {
            ...
            operations.add(
                userRepository.findById(followerId)
                    .flatMap(follower -> {
                        ...
                        UserFeed.FeedItem feedItem = new UserFeed.FeedItem(post);
                        
                        // Add to personal feed only
                        return addToUserFeed(followerId, UserFeed.FeedType.PERSONAL, feedItem);
                    })
                    .onErrorResume(e -> {
                        log.error("Failed to add post {} to follower {} feed: {}", post.getId(), followerId, e.getMessage());
                        return Mono.empty();
                    })
            );
        }
    ...
    private Mono<Void> addToUserFeed(String userId, UserFeed.FeedType feedType, UserFeed.FeedItem feedItem) {
    ...
    private Mono<UserFeed> getUserFeed(String userId, UserFeed.FeedType feedType) {
    ...
    private Mono<Void> backfillUserFeed(String followerId, String followedId) {
        ...
        return Mono.zip(
            ...
            .flatMap(tuple -> {
                ...
                List<UserFeed.FeedItem> feedItems = new ArrayList<>();
                ...
                // Add to personal feed only
                return getUserFeed(followerId, UserFeed.FeedType.PERSONAL)
        ...
        return userFeedRepository.findByUserId(followerId)
            ...
            .flatMap(userFeeds -> {
                ...
                for (UserFeed feed : userFeeds) {
                    boolean modified = feed.getItems().removeIf(item -> unfollowedId.equals(item.getAuthorId()));
            ...
            .onErrorResume(e -> {
                log.error("Failed to remove posts from user {} feed: {}", followerId, e.getMessage());
                return Mono.empty();
            });
    }
}