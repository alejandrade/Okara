...
public class ViewTrackingService {
    ...
    public Mono<Void> recordFeedItemView(String userId, String postId, String postAuthorId, 
                                        ViewEvent.ViewSource viewSource, Long viewDurationMs) {
        ...
        return viewEventRepository.save(viewEvent)
            ...
            .then()
            .doOnSuccess(v -> log.debug("Successfully recorded view for user {} on post {}", userId, postId))
            .onErrorResume(e -> {
                log.error("Failed to record view for user {} on post {}: {}", userId, postId, e.getMessage());
    ...
    private Mono<User> incrementUserViewCount(String userId) {
        return userRepository.findById(userId)
            ...
            .doOnSuccess(user -> log.debug("Incremented view count for user {} to {}", userId, user.getTotalViewsCount()));
    }
